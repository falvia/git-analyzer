...
def analyze_real_git_commits(repo_urls: list[str], company_identifier: str, months_back: int) -> dict:
    ...
    try:
        ...
        try:
            ...
            return {"error": f"An unexpected error occurred during Git version check: {str(e)}"}

        temp_dir = tempfile.mkdtemp()
        
        # Calculate the 'since' date for git log
        since_date = (datetime.datetime.now() - datetime.timedelta(days=months_back * 30)).strftime('%Y-%m-%d %H:%M:%S')
        ...
        for repo_url in repo_urls:
            ...
            try:
                ...
                subprocess.run(['git', 'clone', '--quiet', repo_url, repo_path], check=True, capture_output=True)
                ...
                print(f"An unexpected error occurred during cloning {repo_url}: {str(e)}")
                all_repo_commits_structured.append({
                    "repo_name": repo_name,
                    "repo_url": repo_url,
            ...
            try:
                ...
                result = subprocess.run(git_log_command, check=True, capture_output=True, text=True, encoding='utf-8', errors='ignore')
                ...
                for commit_block in commits_raw:
                    ...
                    if len(parts) >= 5:
                        ...
                        if company_identifier.lower() in author_name.lower() or \
                           company_identifier.lower() in author_email.lower():
        ...
        return {"commit_data": all_repo_commits_structured, "message": "Commit analysis complete."}
...
def main():
    ...
    analysis_result = analyze_real_git_commits(repo_urls, company_identifier, months_back)
